...
import json  # ← Penting: di-import di atas
...
from image_utils import compress_image  # ← Import kompresi

app = Flask(__name__)

# ✅ SET BATAS UPLOAD: 10 MB
...
def ocr_receipt():
    ...
    try:
        # Step 1: Preprocess (kontras, grayscale, resize)
        processed_bytes = preprocess_image(image_bytes)
        
        # Step 2: Kompres ke ukuran < 1 MB (untuk upload ke Firebase & API)
        compressed_bytes = compress_image(processed_bytes, max_size_mb=1.0)
        
        # Step 3: Upload ke Firebase
        image_url = upload_image_to_firebase(compressed_bytes, filename)
        
        # Step 4: Kirim ke Gemini
        prompt = get_ocr_prompt()
        response = model.generate_content([prompt, Image.open(io.BytesIO(compressed_bytes))])
        raw_output = response.text.strip()

        # Bersihkan output (hapus ```json ... ```
        ...
        # Hapus field yang tidak bisa di-JSON-kan
